  ...
  async def podiums(self,ctx):
    ...
    for event in bldevents:
      ...
      for i in range(listlen):
        ...
        usersingle=convert(usersingle)
        if i==0:
          onemessage.append(f"1st place: {user} with an average of {usersingle} and a single of {average}")
          overall[user]+=3
        elif i==1:
          onemessage.append(f"2nd place: {user} with an average of {usersingle} and a single of {average}")
          overall[user]+=2
        elif i==2:
          onemessage.append(f"3rd place: {user} with an average of {usersingle} and a single of {average}")
          overall[user]+=1
      onelen=len(onemessage)
      if onelen>0:
        await ctx.send("\n".join(onemessage))
